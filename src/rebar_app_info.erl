...
-record(app_info_t, {name               :: binary(),									%% App名字
					 app_file_src       :: file:filename_all() | undefined,				%% 当前应用.app.src文件的绝对路径
					 app_file_src_script:: file:filename_all() | undefined,				%% 当前应用.app.src.script文件的绝对路径
					 app_file           :: file:filename_all() | undefined,				%% 当前应用.app文件的绝对路径
					 original_vsn       :: binary() | string() | undefined,				%% 当前应用的版本号
					 parent=root        :: binary() | root,								%% 父依赖项
					 app_details=[]     :: list(),										%% 应用.app,.app.src,.app.src.script文件中的配置信息
					 applications=[]    :: list(),										%% 当前应用依赖的所有应用
					 deps=[]            :: list(),										%% 当前应用的依赖项列表
					 profiles=[default] :: [atom()],									%% 当前应用拥有的profile
					 default=dict:new() :: rebar_dict(),								%% 当前应用对应的rebar.config文件的配置信息
					 opts=dict:new()    :: rebar_dict(),								%% 当前应用对应的rebar.config文件的配置信息
					 dep_level=0        :: integer(),									%% 依赖等级
					 dir                :: file:name(),									%% 当前应用源代码目录
					 out_dir            :: file:name(),									%% 当前应用编译后输出到的目录
					 resource_type      :: pkg | src,									%% 当前应用源的类型
					 source             :: string() | tuple() | checkout | undefined,	%% 当前应用的源
