...
%% 从指定目录查找应用信息，同时将该应用的信息存储到rebar_app_info数据结构中
find_app(AppDir, Validate) ->
...
%% 根据应用的.app,.app.src,.app.src.script文件中的信息初始化rebar_app_info数据结构
...
%% 从指定目录查找应用信息，同时将该应用的信息存储到rebar_app_info数据结构中(查找对应应用的.app,.app.src,.app.src.script文件)
