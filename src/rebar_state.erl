...
-record(state_t, {
				  dir                               :: file:name(),							%% 当前工作目录
				  opts                = dict:new()  :: rebar_dict(),						%% 配置信息
				  code_paths          = dict:new()  :: rebar_dict(),						%% erlang代码的搜索路径字典
				  default             = dict:new()  :: rebar_dict(),
				  escript_path                      :: undefined | file:filename_all(),		%% bootstrap脚本的路径
				  ...
				  deps_to_build       = []          :: [rebar_app_info:t()],				%% 需要重建的依赖项数组
				  all_plugin_deps     = []          :: [rebar_app_info:t()],				%% 所有插件的rebar_app_info数据结构列表
				  all_deps            = []          :: [rebar_app_info:t()],				%% 整个系统的所有依赖项
				  
				  resources           = [],
				  providers           = [],													%% 存储插件信息
				  allow_provider_overrides = false  :: boolean()							%% 是否允许插件被覆盖
				 }).
...
-spec new(list()) -> t().
%% 根据Config配置信息创建rebar_state数据结构
new(Config) when is_list(Config) ->
	%% 创建默认的rebar_state数据结构
    BaseState = base_state(),
	%% 根据Config创建rebar_state结构中opts字典数据结构
...
%% 将ParentState和Config合并组装新的rebar_state数据结构
new(ParentState, Config, Dir) ->
...
%% 从config配置中得到deps依赖项，如果有rebar.lock文件，则从该文件拿到依赖项
...
%% 增加新的Profile，将该profile对应的配置和opts中的配置合并
